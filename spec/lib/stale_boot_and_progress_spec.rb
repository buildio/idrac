...
RSpec.describe "IDRAC::Boot stale entries and boot progress" do
  ...
  describe "#disable_boot_entries" do
    ...
    # Adapted from #19: the merged spine defaults to wait: false (the caller's reboot applies the
    # change). wait: true polls the scheduled BIOS config job to a terminal state via wait_config_job.
    it "polls the scheduled config job to a terminal state when wait: true (reuses wait_config_job)" do
      stub_request(:get, %r{/Systems/System\.Embedded\.1/BootSources$})
      ...
      expect(client.disable_boot_entries(wait: true, timeout: 5)).to eq(["Unknown.Unknown.1-1"])
      expect(a_request(:get, %r{/Managers/iDRAC\.Embedded\.1/Jobs/JID_555})).to have_been_made
    end

    it "does not poll the config job when wait is left at its default (false)" do
      stub_request(:get, %r{/Systems/System\.Embedded\.1/BootSources$})
        .to_return(status: 200, body: boot_sources(["Unknown.Unknown.1-1", true]).to_json)
      stub_request(:patch, %r{/Systems/System\.Embedded\.1/BootSources/Settings$}).to_return(status: 200, body: '{}')

      client.disable_boot_entries
      expect(a_request(:get, %r{/Managers/iDRAC\.Embedded\.1/Jobs/JID_555})).not_to have_been_made
    end
  ...
  # Adapted from #19's #normalize_boot_progress unit tests. The merged spine (#20) folds the
  # normalization into #boot_progress via the BOOT_PROGRESS_STATES table and exposes no separate
  # normalize_boot_progress method, so the same enum -> symbol mapping is asserted through the public
  # #boot_progress instead of deleting the coverage.
  describe "#boot_progress state mapping" do
    {
      "OSRunning" => :os_running,
      "OSBootStarted" => :os_boot_started,
      "SetupEntered" => :setup_entered,
      "None" => :none
    }.each do |last_state, symbol|
      it "maps Redfish LastState #{last_state.inspect} to #{symbol.inspect}" do
        stub_request(:get, %r{/Systems/System\.Embedded\.1\?\$select=BootProgress})
          .to_return(status: 200, body: { "BootProgress" => { "LastState" => last_state } }.to_json)
        expect(client.boot_progress).to eq(symbol)
      end
    end

    it "returns nil when LastState is absent" do
      stub_request(:get, %r{/Systems/System\.Embedded\.1\?\$select=BootProgress})
        .to_return(status: 200, body: { "BootProgress" => {} }.to_json)
      expect(client.boot_progress).to be_nil
    end
  end
end
